...
export class addParamsToUser1643066650828 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "user" ADD "profile_image" character varying`,
    );
    await queryRunner.query(
      `ALTER TABLE "user" ADD "status" character varying NOT NULL DEFAULT 'offline'`,
    );
    await queryRunner.query(
      `ALTER TABLE "user" ADD "status_message" character varying`,
    );
  }
